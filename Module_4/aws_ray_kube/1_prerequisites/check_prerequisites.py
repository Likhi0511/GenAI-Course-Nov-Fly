...
2_cloudformation/
└── eks-parameters.json  # Updated with ECR URI and secret ARNs
...
Fix:   Edit 2_cloudformation/eks-parameters.json and set unique bucket name
...
- EKS pods retrieve secrets at runtime as environment variables (Kubernetes Secrets)
...
- Verify region supports EKS (most regions do)

==============================================================================
NEXT STEPS AFTER SUCCESS
==============================================================================
Once all checks pass:
1. Review eks-parameters.json to verify all parameters are set
...
- FIX 2: Corrected CloudFormation template filename in Check 10.
         Now looks for 'eks-cluster.yaml'
...
def check_eks_tools():
    """
    Check EKS tools: verify kubectl and helm are installed.
    Both are required for EKS deployment.
      - kubectl: talks to the EKS cluster (apply manifests, get logs)
      - helm: installs KubeRay operator and Cluster Autoscaler
    Not needed for the Docker build (Check 8) but needed before deploy.sh runs.
    """
    print("\n[ 4b] kubectl + helm (EKS tools)")
    ...
    for tool, install_hint in [
        ...
        cmd = [tool, "version", "--client"] if tool == "kubectl" else [tool, "version", "--short"]
        code, out, _ = run(cmd)
        if code == 0:
            version = out.split("\n")[0].strip()
            passed(f"{tool}: {version}")
        else:
            failed(f"{tool} not found")
            fix(f"Install: {install_hint}")
...
# covers all services needed: CloudFormation, ECR, EKS, S3, DynamoDB,
...
def check_aws_permissions(region):
    ...
    services = [
        ...
        ("EKS",            ["aws", "eks", "list-clusters", "--max-results", "1"]),
        ("S3",             ["aws", "s3", "ls"]),
...
def check_and_provision_secrets(region):
    ...
    provision them in Secrets Manager, and update eks-parameters.json.
    """
    ...
    # Update eks-parameters.json with the secret ARNs
    if param_updates:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        params_file = os.path.normpath(os.path.join(
            script_dir, "..", "2_cloudformation", "eks-parameters.json"
        ))
            ...
            try:
                ...
                if isinstance(params, list):
                    ...
                    for key, value in param_updates.items():
                        ...
                        info(f"{'Updated' if found else 'Added'} eks-parameters.json → {key}")

                elif isinstance(params, dict) and "Parameters" in params:
                    # Object format: {"Parameters": {"Key": "Value", ...}}
                    for key, value in param_updates.items():
                        params["Parameters"][key] = value
                        info(f"Updated eks-parameters.json → {key}")

                else:
                    failed("Unexpected format in eks-parameters.json")
                    return
                ...
                failed(f"Could not update eks-parameters.json: {e}")
...
def check_s3_bucket_name(region):
    """Verify S3 bucket name is configured in eks-parameters.json."""
    print("\n[ 7 ] S3 Bucket Name")
    ...
    if not os.path.isfile(params_file):
        failed("eks-parameters.json not found")
        return
    ...
    try:
        ...
        if bucket_name in ("your-unique-bucket-name-here", "my-document-pipeline-prod-12345", ""):
            failed("S3BucketName is still a placeholder value in eks-parameters.json")
            fix("Edit 2_cloudformation/eks-parameters.json")
        ...
        failed(f"Could not read eks-parameters.json: {e}")
...
def _prefill_yaml_placeholders(region: str, ecr_uri: str, script_dir: str):
    """
    Pre-fill the static placeholders in ray-cluster.yaml and k8s-supporting.yaml
    that are known BEFORE CloudFormation runs.

    Fills now (check_prerequisites.py — after Docker build):
      ray-cluster.yaml:    <ECR_IMAGE_URI>  → actual ECR URI
                           <AWS_REGION>     → actual region

    Filled later (step1b_setup_eks.py — after CloudFormation runs):
      k8s-supporting.yaml: <RAY_POD_ROLE_ARN>   → CFN output RayPodRoleArn
      ray-cluster.yaml:    <S3_BUCKET>           → CFN output S3BucketName
                           <CONTROL_TABLE>       → CFN output ControlTableName
                           <AUDIT_TABLE>         → CFN output AuditTableName
                           <METRICS_TABLE>       → CFN output MetricsTableName
    """
    cfn_dir = os.path.normpath(os.path.join(script_dir, "..", "2_cloudformation"))
    ecr_uri_latest = f"{ecr_uri}:latest"

    targets = {
        "ray-cluster.yaml": {
            "<ECR_IMAGE_URI>": ecr_uri_latest,
            "<AWS_REGION>":    region,
        }
        # k8s-supporting.yaml has only <RAY_POD_ROLE_ARN> which requires CFN output —
        # step1b fills it at runtime after the stack deploys.
    }

    for filename, replacements in targets.items():
        filepath = os.path.join(cfn_dir, filename)
        if not os.path.isfile(filepath):
            warn(f"  {filename} not found at {filepath} — skipping pre-fill")
            continue
        ...
        try:
            ...
            for placeholder, value in replacements.items():
                if placeholder in content_yaml:
                    content_yaml = content_yaml.replace(placeholder, value)
            ...
            if changed:
                ...
                for ph in changed:
                    info(f"  Pre-filled {filename}: {ph} → {replacements[ph]}")
            else:
                info(f"  {filename}: placeholders already filled or not present")

        except IOError as e:
            warn(f"  Could not update {filename}: {e}")
...
def build_and_push_docker(region: str, account_id: str):
    ...
    On success:
      - Updates eks-parameters.json with the ECR URI
      - Pre-fills <ECR_IMAGE_URI> and <AWS_REGION> in ray-cluster.yaml
        (CFN-dependent placeholders like <RAY_POD_ROLE_ARN> are filled later by step1b)
    """
    ...
    # Detect Apple Silicon — must build for linux/amd64 to run on EKS (EC2 x86_64 nodes)
    ...
    # ── Update eks-parameters.json with ECR URI ─────────────────
    params_file = os.path.normpath(os.path.join(
        script_dir, "..", "2_cloudformation", "eks-parameters.json"
    ))
    ...
    if os.path.isfile(params_file):
        try:
            ...
            info("Updated eks-parameters.json → ECRImageUri")
            passed("ECR URI written to eks-parameters.json")
    ...
    # ── Pre-fill static placeholders in YAML manifests ───────────────────────
    # ECR_IMAGE_URI and AWS_REGION are known now — fill them into ray-cluster.yaml.
    # CFN-dependent values (<RAY_POD_ROLE_ARN>, table names, S3 bucket) are filled
    # later by step1b_setup_eks.py after CloudFormation completes.
    info("Pre-filling static placeholders in Kubernetes manifests...")
    _prefill_yaml_placeholders(region, ecr_uri, script_dir)
...
def check_aws_service_quotas(region: str):
    ...
    info("Network design: public subnets + IGW — EKS nodes get public IPs (no NAT needed for demo)")
    info("EKS nodes get public IPs directly — zero extra networking cost")

    # ── Informational: ECS Task Quota ───────────────────────────────────────
    info("EKS: EC2 instance vCPU quota applies — default 32 on-demand vCPUs/region (enough for 4x m5.4xlarge nodes)")
...
def check_cloudformation_template(region: str):
    ...
    to 'eks-cluster.yaml' to match the EKS CloudFormation template.
    ...
    template_path = os.path.normpath(os.path.join(
        script_dir, "..", "2_cloudformation", "eks-cluster.yaml"
    ))

    if not os.path.isfile(template_path):
        failed(f"CloudFormation template not found: {template_path}")
        fix("Expected file: 2_cloudformation/eks-cluster.yaml")
    ...
    if code == 0:
        ...
        if err:
            ...
            for line in err.split("\n")[:5]:
                if line.strip():
                    fix(line.strip())
        fix("Fix template errors in: 2_cloudformation/eks-cluster.yaml")
...
def main():
    ...
    check_eks_tools()                                      # Check 4b (EKS tools)
    ...
    if FAIL_COUNT == 0:
        ...
        print("  ✓ eks-parameters.json fully populated (ECRImageUri, SecretARNs)")
        print("  ✓ ray-cluster.yaml pre-filled (<ECR_IMAGE_URI>, <AWS_REGION>)")
        print("  ℹ  k8s-supporting.yaml: <RAY_POD_ROLE_ARN> filled by step1b after CFN")
        print("  ℹ  ray-cluster.yaml: table/S3 placeholders filled by step1b after CFN")
        print()
        print("  NEXT: Deploy the CloudFormation stack")
        print("  Run:  python3 2_cloudformation/orchestrator.py")
    else:
