"""
check_windows.py
Ray Document Processing Pipeline — Prerequisites Check (Windows Edition)

This is the Windows-specific version of check.py with additional fixes:
  - Sets console codepage to UTF-8 (chcp 65001) at startup
  - Enables ANSI color codes on Windows 10+ via ctypes
  - Forces PYTHONIOENCODING=utf-8 to prevent charmap errors in child processes
  - All subprocess calls use encoding='utf-8' + errors='replace'
  - Includes Windows-specific Docker Desktop troubleshooting tips

If you're on macOS or Linux, use check.py instead (lighter, same checks).
...
On Windows:
    python check.py
    python check_windows.py   (preferred — includes extra Windows fixes)
...
├── check.py                       # This script (cross-platform)
├── check_windows.py               # Windows-specific version with extra fixes
...
└── sitecustomize.py               # UTF-8 encoding fix for container
...
Fix:   Edit deploy/cloudformation/cloudformation-parameters.json and set unique bucket name

Issue: "'charmap' codec can't decode byte" (Windows)
Fix:   Use check_windows.py instead, or run: chcp 65001 before running this script
...
Fixes in 2.2:
- FIX 4: Added encoding='utf-8' + errors='replace' to ALL subprocess calls.
         On Windows, Python defaults to cp1252 which crashes on Docker's
         Unicode progress bars (━, █, ▏) with 'charmap' codec error.
         Now works on all Windows locales without chcp 65001.
- FIX 5: Created check_windows.py with additional Windows-specific fixes:
         chcp 65001 console codepage, ANSI color via ctypes, Docker Desktop tips.
- Restructured: Renamed from check_prerequisites.py → check.py
         Updated directory references to new package-based layout.
...
# ═════════════════════════════════════════════════════════════════════════════
# WINDOWS-SPECIFIC SETUP (runs before anything else)
# ═════════════════════════════════════════════════════════════════════════════
# Three fixes that prevent the 'charmap' UnicodeDecodeError on Windows:
#
# 1. chcp 65001 — Switch the Windows console from cp1252 to UTF-8.
#    Without this, Docker build output (which contains Unicode progress bars
#    like ━, █, ▏) crashes: "'charmap' codec can't decode byte 0x..."
#
# 2. PYTHONIOENCODING=utf-8 — Forces Python's stdin/stdout/stderr to use
#    UTF-8 encoding in this process AND any child processes (subprocess).
#    Without this, print() can crash on Unicode characters even after chcp.
#
# 3. ENABLE_VIRTUAL_TERMINAL_PROCESSING — Tells Windows 10+ conhost.exe
#    to interpret ANSI escape codes (\033[0;32m) as colors instead of
#    printing them as raw text.  This lets us show colored [PASS]/[FAIL].
#
# These three fixes together make the script work identically on:
#   - Windows 10/11 CMD
...
if IS_WINDOWS:
    # Fix 1: Switch console codepage to UTF-8
    try:
        os.system("chcp 65001 > nul 2>&1")
    except Exception:
        pass

    # Fix 2: Force UTF-8 for Python's IO and child processes
    ...
    # Fix 3: Enable ANSI escape codes on Windows 10+
    # The Windows console (conhost.exe) supports ANSI colors since Windows 10
    # version 1511, but it must be explicitly enabled via SetConsoleMode().
    try:
        import ctypes
        ...
        _windows_ansi_enabled = (result != 0)
    except Exception:
        _windows_ansi_enabled = False
...
# ═════════════════════════════════════════════════════════════════════════════
IS_WINDOWS = platform.system() == "Windows"
# On Windows, use ANSI colors if we successfully enabled VT processing above.
# On other platforms, use colors if stdout is a terminal (not redirected to file).
USE_COLOR  = _windows_ansi_enabled if IS_WINDOWS else sys.stdout.isatty()
...
def run(cmd: list) -> tuple[int, str, str]:
    ...
    """
    try:
        # encoding='utf-8' + errors='replace' prevents UnicodeDecodeError
        # on Windows where the default codec is cp1252 and Docker/AWS CLI
        # output contains Unicode characters (progress bars, check marks).
        r = subprocess.run(
...
def check_docker():
    ...
    if code == 0:
        ...
        if IS_WINDOWS:
            ...
            fix("If Docker Desktop won't start:")
            fix("  1. Open 'Turn Windows features on or off'")
            fix("  2. Enable 'Windows Subsystem for Linux' and 'Virtual Machine Platform'")
            fix("  3. Restart your PC, then start Docker Desktop again")
            fix("  4. If using WSL2 backend: wsl --update  (from admin PowerShell)")
        elif platform.system() == "Darwin":
...
def check_and_provision_secrets(region):
    ...
    for cfg in secrets_config:
        ...
        if code == 0:
        ...
        elif "ResourceExistsException" in err or "ResourceExistsException" in out:
            # Secret exists but describe-secret failed earlier (timeout/throttle).
            # Fetch the ARN directly — we know the secret is there.
            info(f"  Secret already exists (describe timed out). Fetching ARN...")
            code2, out2, err2 = run([
                "aws", "secretsmanager", "describe-secret",
                "--secret-id", secret_name,
                "--region", region,
            ...
            if code2 == 0:
                try:
                    secret_data = json.loads(out2)
                    secret_arn  = secret_data.get("ARN")
                    passed(f"{env_var} — secret already exists in Secrets Manager")
                    info(f"  ARN: {secret_arn}")
                    param_updates[param_key] = secret_arn
                except (json.JSONDecodeError, KeyError):
                    failed(f"{env_var} — secret exists but could not parse ARN")
            else:
                failed(f"{env_var} — secret exists but could not retrieve ARN")
        else:
    ...
    if param_updates:
        ...
        params_file = os.path.normpath(os.path.join(
            script_dir, "..", "cloudformation", "cloudformation-parameters.json"
        ))
...
def check_s3_bucket_name(region):
    ...
    try:
        ...
        if bucket_name in ("your-unique-bucket-name-here", "my-document-pipeline-prod-12345", ""):
            ...
            fix("Edit deploy/cloudformation/cloudformation-parameters.json")
...
def build_and_push_docker(region: str, account_id: str):
    ...
    # ── Locate Dockerfile ───────────────────────────────────────────────────
    # Dockerfile lives in docker/, but the build context must be the project
    # root so COPY can reach core/, stages/, orchestration/ packages.
    script_dir     = os.path.dirname(os.path.abspath(__file__))
    ...
    info(f"Building image (linux/amd64) — context: {project_root}")
    ...
    # Build context is project root so COPY core/ stages/ orchestration/ works.
    # -f points to docker/Dockerfile inside the context.
    is_arm_mac = (platform.system() == "Darwin" and platform.machine() == "arm64")
    build_cmd  = ["docker", "buildx", "build","--no-cache", "-f", dockerfile]
    if is_arm_mac:
        build_cmd += ["--platform", "linux/amd64"]
    build_cmd += ["-t", local_tag, project_root]
    ...
    try:
        process = subprocess.Popen(
            ...
            text=True,
            bufsize=1,
            encoding="utf-8",   # FIX: Windows cp1252 can't decode Docker's Unicode progress bars
            errors="replace",   # Replace un-decodable bytes with ? instead of crashing
        )
    ...
    for tag in tags:
        ...
        process = subprocess.Popen(
            ...
            text=True,
            bufsize=1,
            encoding="utf-8",   # FIX: Windows cp1252 charmap fix
            errors="replace",
        )
        for line in process.stdout:
            print(line, end="")
...
def _ecr_login(region: str, account_id: str) -> bool:
    ...
    result   = subprocess.run(
        ...
        encoding="utf-8", errors="replace",  # FIX: Windows cp1252 charmap fix
    )
...
def check_cloudformation_template(region: str):
    ...
    template_path = os.path.normpath(os.path.join(
        script_dir, "..", "cloudformation", "1_ray-pipeline-cloudformation-public.yaml"
    ))

    if not os.path.isfile(template_path):
        failed(f"CloudFormation template not found: {template_path}")
        fix("Expected file: deploy/cloudformation/1_ray-pipeline-cloudformation-public.yaml")
    ...
    if code == 0:
        ...
        if err:
            ...
            for line in err.split("\n")[:5]:
                if line.strip():
                    fix(line.strip())
        fix("Fix template errors in: deploy/cloudformation/1_ray-pipeline-cloudformation-public.yaml")
...
def main():
    ...
    print()
    print("=" * 60)
    print("  RAY PIPELINE — PREREQUISITES CHECK  v2.2")
    print(f"  Platform: {platform.system()} {platform.release()}")
    if IS_WINDOWS:
        print(f"  Console : {'UTF-8 (chcp 65001)' if os.environ.get('PYTHONIOENCODING') == 'utf-8' else 'system default'}")
        print(f"  Colors  : {'ANSI enabled' if _windows_ansi_enabled else 'disabled (plain text)'}")
    print("=" * 60)
