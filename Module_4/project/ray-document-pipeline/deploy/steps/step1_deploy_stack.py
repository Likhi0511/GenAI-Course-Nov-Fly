...
Reads:     deploy/cloudformation/template.yaml
...
# Resolve paths relative to this script's directory (deploy/steps/)
# Template and parameters live in deploy/cloudformation/
_SCRIPT_DIR   = Path(__file__).resolve().parent
_CFN_DIR      = _SCRIPT_DIR.parent / "cloudformation"
TEMPLATE_FILE = str(_CFN_DIR / "1_ray-pipeline-cloudformation-public.yaml")
PARAMS_FILE   = str(_CFN_DIR / "cloudformation-parameters.json")
...
def main():
    ...
    if success:
        ...
        print("=" * 70 + "\n")
        print("Next: Run step2_download_pdfs.py\n")
        sys.exit(0)
    else:
        print("\n❌ Step 1 Failed\n")
        print("Fix the issue and re-run: python step1_deploy_stack.py\n")
        sys.exit(1)
